...
import { analyzeGameStream, getCoaching } from '../api/chess'
...
export default function Analysis() {
  ...
  // Build full analysis object for coaching
  const fullAnalysis = gameMeta && summary ? {
    ...gameMeta,
    moves: streamedMoves,
    summary,
  } : null
        ...
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-400">
              {game.result} · {game.time_control} · {gameMeta?.opening || game.opening || 'Unknown opening'}
            </p>
          </div>
          {/* Depth + Analyze */}
        ...
        <div className="flex gap-6">
          ...
          <div className="flex gap-3 flex-shrink-0">
            ...
            <div className="flex flex-col gap-3">
              {/* Opponent label */}
              <div className="flex items-center gap-2">
                <div className={`w-5 h-5 rounded ${playerColor === 'white' ? 'bg-gray-800 border border-gray-600' : 'bg-gray-200'}`} />
                <span className="text-sm text-gray-300">{opponent}</span>
              </div>

              {/* Board */}
              <div className="w-96 rounded-xl overflow-hidden border border-gray-700 shadow-2xl">
                <Chessboard
                  key={boardPosition}
                  options={{
              ...
              {/* Player label */}
              <div className="flex items-center gap-2">
                <div className={`w-5 h-5 rounded ${playerColor === 'white' ? 'bg-gray-200' : 'bg-gray-800 border border-gray-600'}`} />
                <span className="text-sm text-gray-300">{username || (playerColor === 'white' ? game.white : game.black)}</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 text-center text-xs text-gray-400">
                  {currentIndex < 0
                    ? <span>Start</span>
                    : <span>Move {currentMove?.move_number} <span className="font-mono text-gray-200">{currentMove?.move_san}</span></span>
                  }
                </div>
              ...
              {currentMove && (
                <div className="flex items-center justify-between gap-3">
                  <div className={`px-3 py-1 rounded-full text-xs font-semibold ${CLASSIFICATION_BADGE[currentMove.classification]?.color}`}>
                    {CLASSIFICATION_BADGE[currentMove.classification]?.label}
                  </div>
                  {currentMove.best_move_san && currentMove.classification !== 'best' && (
                    <div className="text-xs text-gray-400">
                      Best: <span className="font-mono text-emerald-400">{currentMove.best_move_san}</span>
                    </div>
                  )}
                  {currentEval !== null && (
                    <div className={`text-xs font-mono font-bold ${currentEval >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
              ...
              {analyzing && streamedMoves.length > 0 && (
                ...
                >
                  {trackLatest ? '📍 Tracking latest' : '📌 Click to track latest'}
                </button>
              )}
              ...
              {!analyzing && streamedMoves.length === 0 && (
                <div className="text-center text-xs text-gray-500 py-2">
                  Press <span className="text-blue-400">▶ Analyze</span> to start
                </div>
              )}
            </div>
          </div>
