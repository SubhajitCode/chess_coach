...
export default function MoveTable({
  ...
  useEffect(() => {
    ...
    if (activeRect.height === 0 && containerRect.height === 0) {
      return
    }

    const offsetTopDiff = activeRect.top - containerRect.top
    const offsetBottomDiff = activeRect.bottom - containerRect.bottom

    if (offsetTopDiff < 0) {
      if (typeof container.scrollBy === 'function') {
        container.scrollBy({ top: offsetTopDiff - 8, behavior: 'smooth' })
      } else {
        container.scrollTop += offsetTopDiff - 8
      }
    } else if (offsetBottomDiff > 0) {
      if (typeof container.scrollBy === 'function') {
        container.scrollBy({ top: offsetBottomDiff + 8, behavior: 'smooth' })
      } else {
        container.scrollTop += offsetBottomDiff + 8
      }
    }
  }, [currentIndex])
